...
class TaskMangae {
    ...
    public function __construct()
    {
        ...
        add_action( 'wp_ajax_delete_year_setting', array($this,'delete_year_setting'));
        add_action( 'wp_ajax_nopriv_delete_year_setting',array($this,'delete_year_setting'));

        add_action( 'wp_ajax_send_mail_multiple', array($this,'send_mail_multiple'));
        add_action( 'wp_ajax_nopriv_send_mail_multiple',array($this,'send_mail_multiple'));
       
        add_action( 'wp_ajax_update_start_end_multiple', array($this,'update_start_end_multiple'));
        add_action( 'wp_ajax_nopriv_update_start_end_multiple',array($this,'update_start_end_multiple'));

        add_action( 'wp_ajax_delete_multiple_record', array($this,'delete_multiple_record'));
        add_action( 'wp_ajax_nopriv_delete_multiple_record',array($this,'delete_multiple_record'));

        add_action( 'wp_ajax_update_start_value', array($this,'update_start_value'));
        add_action( 'wp_ajax_nopriv_update_start_value',array($this,'update_start_value'));  

        add_action( 'wp_ajax_update_end_value', array($this,'update_end_value'));
        add_action( 'wp_ajax_nopriv_update_end_value',array($this,'update_end_value'));
       ...
       // add_action('wp_login', array($this,'Redirect_control_page'),10,2);
        ...
        add_shortcode( 'broadcast_dashboard', array($this, 'Broadashboard'));
        add_shortcode( 'singletasllist_gridview', array($this, 'Singletasklistgrid'));
       ...
       // add_action('wp_login',array($this,'backend_redirect'));
     ...
     function register_api_hooks() {
      ...
      register_rest_route(
        'taskmanager/v1', '/allphotographer/',
        array(
          'methods'  => 'GET',
          'callback' => 'allphotographer',
        )
      ); 

        register_rest_route(
        'taskmanager/v1', '/singlephotographer/(?P<id>\d+)',
        array(
          'methods'  => 'GET',
          'callback' => 'singlephotographer',
        )
      );
    ...
    function update_start_value(){
    global $wpdb;
    $resu ="";
      $table = $wpdb->prefix.'schedule_task';
    $start_value = $_POST['start_value'];
    $photo_id = $_POST['photo_id'];
    $curr_grab_id = $_POST['curr_grab_id'];
    $change_start = $_POST['change_start'];
    $allresul = $wpdb->get_results("select * from $table where photo_gra_ID ='".$curr_grab_id."' AND  date='".$change_start."'");
    //print_r($allresul);
      foreach ($allresul as $key => $value){
            $mytimes =$value->start_time;
            $starts_array[]= $mytimes;
      }
      //print_r($starts_array);
      if (in_array($start_value, $starts_array, TRUE))
        {
         $resu = "1";
        }
       else
         {
          $updated= $wpdb->query($wpdb->prepare("UPDATE $table SET start_time='$start_value' WHERE Id=$photo_id"));
                if ( false === $updated ) {
                    $resu ="error";
                } else {
                    $resu ="sucess";
...
function update_end_value(){
    global $wpdb;
    $resu ="";
      $table = $wpdb->prefix.'schedule_task';
    $end_value = $_POST['end_value'];
    $photo_end_id = $_POST['photo_end_id'];
    $curr_grab_id = $_POST['curr_grab_id'];
    $change_start = $_POST['change_start'];
    $allresul = $wpdb->get_results("select * from $table where photo_gra_ID ='".$curr_grab_id."' AND  date='".$change_start."'");
    //print_r($allresul);
      foreach ($allresul as $key => $value){
            $mytimes =$value->end_time;
            $starts_array[]= $mytimes;
      }
      //print_r($starts_array);
      if (in_array($end_value, $starts_array, TRUE))
        {
         $resu = "1";
        }
       else
         {
          $updated= $wpdb->query($wpdb->prepare("UPDATE $table SET end_time='$end_value' WHERE Id=$photo_end_id"));
                if ( false === $updated ) {
                    $resu = "error";
                } else {
                    $resu = "sucess";
...
function backend_redirect(){
    ...
    if(is_user_logged_in()):
      $current_user = wp_get_current_user();
		...
		var url = "<?php echo home_url( $wp->request);?>";
		//window.location.href="<?php echo  home_url( $wp->request ).'/control/?id=hhhhdgd123';   ?>";
		 </script>
		 <?php  endif;
...
public function schedule_popup()
{
  if(!is_admin())
      ...
      public function Broadashboard()
        {
        if(!is_admin())
        {
          ob_start(); 
       include(plugin_dir_path( __FILE__ ).'include/broadcast_dashboard.php');

        }
        }

         public function Singletasklistgrid()
        {
        if(!is_admin())
        {
          ob_start(); 
       include(plugin_dir_path( __FILE__ ).'include/single-tasklist.php');
...
for($i=0;$i<count($checkbox);$i++){
        $del_id = $checkbox[$i]; 
  $delete =  $wpdb->query("DELETE FROM $table WHERE id IN($del_id)");
    }
//$delete =$wpdb->delete( $table, array( 'id' => $_POST['id'] ) );
  if ( false === $delete ) {
    echo  "error";
...
function update_start_end_multiple(){
  global $wpdb;
  $table=$wpdb->prefix.'schedule_task';
...
function send_mail_multiple(){
  ...
  $offset= 4*60*60;
    $message .= '<html>
                        <head>
                          <title>Upated task schedule</title>
                        </head>
                        <body>
                        <h4 style="text-align:center; font-size:22px; margin-bottom:10px; text-transform: uppercase;">UPDATED SCHEDULE - ('.gmdate("H:i",time()+$offset).')  '.date("M d Y ").'</h4>
                          <table class="schedule_format" style="margin: 0 auto; width: 100%;">
                            <tr style="background-color: #dd0000; color:#fff; font-size:14px; padding: 12px 12px;">
                              <th style="padding: 10px 10px;">Photographer</th><th colspan="3" style="padding: 10px 10px;">Date</th><th style="padding: 10px 10px;">Start Time</th><th style="padding: 10px 10px;">End Time</th><th style="padding: 10px 10px;">Task</th><th style="padding: 10px 10px;">location</th><th style="padding: 10px 10px;">Notes</th>
                            </tr>';
    foreach ($checkbox as $values) {
    $sendmails = $wpdb->get_results("select * FROM $table WHERE photo_gra_ID = '".$values."' ");
    foreach ($sendmails as $key => $value) {
        $date[$key]  = $value->date;
    $start_time[$key] = $value->start_time;
}
    array_multisort($date, SORT_ASC, $start_time, SORT_ASC, $sendmails);
    foreach ($sendmails as $key => $value) {
      $photo_id = array($value->photo_gra_ID);
      $phot = array_unique($photo_id);
      $headers[] = 'From: SITENAME <hello@domain.com>';

            $subject = "Task Schedule: ".get_the_title($value->photo_gra_ID)."";
              
            $message .= '<tr>
            
                              <td style="text-align: center; background-color: #f2f2f2; color: #000; padding: 10px 12px; font-size: 13px;">'. get_the_title($value->photo_gra_ID).'</td><td colspan="3" style="white-space:nowrap; text-align: center; background-color: #f2f2f2; color: #000; padding: 10px 12px; font-size: 13px;">'.date("M, d, Y ", strtotime($value->date)).'</td><td style="text-align: center; background-color: #f2f2f2; color: #000; padding: 10px 12px; font-size: 13px;">'.$value->start_time.'</td><td style="text-align: center; background-color: #f2f2f2; color: #000; padding: 10px 12px; font-size: 13px;">'.$value->end_time.'</td><td style="text-align: center; background-color: #f2f2f2; color: #000; padding: 10px 12px; font-size: 13px;">'. get_the_title($value->task_ID).'</td><td style="text-align: center; background-color: #f2f2f2; color: #000; padding: 10px 12px; font-size: 13px;">'.get_the_title($value->location_ID).'</td><td style="text-align: center; background-color: #f2f2f2; color: #000; padding: 10px 12px; font-size: 13px;">'.$value->task_notes.'</td>
                            </tr>';
    }
    }
    $message .= '</table>
    ...
    $email_add = get_field('email_address',$phot[0]);
    $headers = array('Content-Type: text/html; charset=UTF-8');
  $send = wp_mail($email_add, $subject, $message, $headers);
    if ( false === $send ) {
      echo  "error";
  } else {
      echo "sucess";
...
function Timemange()
{
    ...
    if(isset($_POST['photographer_id']) && !empty($_POST['photographer_id'])):
        global $wpdb;
      $table = $wpdb->prefix.'schedule_task';
      //echo $_POST['photographer_id'];
      $photogra_id = json_decode(stripslashes($_POST['photographer_id']));
      
      foreach ($photogra_id as $valuess) {
        //echo $valuess;
      
      //echo "select * from $table where photo_gra_ID ='".$valuess."' AND  date='".$_POST['currentdate']."'";
      $allresults = $wpdb->get_results("select * from $table where photo_gra_ID ='".$valuess."' AND  date='".$_POST['currentdate']."'");
...
{   
    ...
    add_submenu_page('edit.php?post_type=photographer','Setting','Setting','manage_options','Setting',array($this,'setting_page'));
    }
